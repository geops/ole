...
OpenLayers.Editor.Control.LayerSettings =  OpenLayers.Class(OpenLayers.Control, {
    ...
    initialize: function(options) {
        ...
        window.onload = OpenLayers.Function.bind(this.domLoaded, this);

    },

    domLoaded: function() {

        this.layerSwitcher = this.map.getControlsByClass('OpenLayers.Control.LayerSwitcher')[0];
    ...
    showLayerSettings: function(layerName) {
        ...
        */
        ...
        this.map.editor.dialog.show(content, {title: layerName});
    },

    toggleExportFeature: function() {
        if(this.currentLayer.exportFeature) {
            this.currentLayer.exportFeature = false;
        } else {
            this.currentLayer.exportFeature = true;
        }
    },
