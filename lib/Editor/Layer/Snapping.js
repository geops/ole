OpenLayers.Editor = OpenLayers.Editor || { };
OpenLayers.Editor.Layer = OpenLayers.Editor.Layer || { };
...
OpenLayers.Editor.Layer.Snapping = OpenLayers.Class(OpenLayers.Layer.Vector, {
    ...
    addLine: function (line) {
        ...
        if (line.m === Infinity || line.m === -Infinity) {
            ...
            } else if (typeof line.y !== 'undefined') {
                guideLine = new OpenLayers.Feature.Vector(
                    new OpenLayers.Geometry.LineString([
                        new OpenLayers.Geometry.Point(maxExtent.right, line.y),
                        new OpenLayers.Geometry.Point(maxExtent.left, line.y)
                    ])
                );
            } else {
                throw new Error('line.x AND line.y both undefined');
            }
        } else {
            guideLine = new OpenLayers.Feature.Vector(
                new OpenLayers.Geometry.LineString([
        ...
        if (!guideLine) {
            throw new Error('guideLine undefined');
        }
        this.addFeatures([
            guideLine
        ]);
    ...
    rebuildIntersectionPoints: function () {
        ...
        this.features.forEach(function (feature) {
            ...
            segments.forEach(function (seg1) {
                this.features.forEach(function (featureOther) {
                    ...
                    segmentsOther.forEach(function (seg2) {
                        var intersection = OpenLayers.Geometry.segmentsIntersect(seg1, seg2, intersectionOptions);
                        var valid = intersection.x && intersection.y
                            && intersection.x >= maxExtent.left && intersection.x <= maxExtent.right
                            && intersection.y >= maxExtent.bottom && intersection.y <= maxExtent.top;
                        if (!valid) {
                            return;
                        }
    ...
    drawFeature: function (feature, style) {
        ...
        if (line.m === Infinity || line.m === -Infinity) {
            if (typeof line.x !== 'undefined') {
                ...
                viewportBoundaryFirst = new OpenLayers.Geometry.Point(viewportExtent.right, line.y);
                viewportBoundarySecond = new OpenLayers.Geometry.Point(viewportExtent.left, line.y);
            } else {
                throw new Error('line.x AND line.y both undefined');
            }
        } else {
